...
# globals
DATA_DIR = "%s/data/blueline" % (os.path.dirname(os.path.realpath(__file__)))


@pytest.mark.blueline
...
def test_read_single_file(test_dict):
    # read file
    img, meta, problematic_files = trex_imager_readfile.read_blueline("%s/%s" % (DATA_DIR, test_dict["filename"]))
    ...
    assert img.shape == (270, 320, test_dict["expected_frames"])
...
def test_read_multiple_files(test_dict):
    ...
    # read file
    img, meta, problematic_files = trex_imager_readfile.read_blueline(file_list)
...
def test_read_single_file_workers(test_dict):
    # read file
    img, meta, problematic_files = trex_imager_readfile.read_blueline(
