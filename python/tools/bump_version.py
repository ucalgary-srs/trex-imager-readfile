...
def main():
    # args
    parser = argparse.ArgumentParser(description="Bump the version number for the trex-imager-readfile library")
    ...
    print("Updating trex_imager_readfile/__init__.py file ...")
    src_filename = "%s/../trex_imager_readfile/__init__.py" % (os.path.dirname(os.path.realpath(__file__)))
    dst_filename = "%s/../trex_imager_readfile/__init__.py.bak" % (os.path.dirname(os.path.realpath(__file__)))
    try:
        # open files for read/write
        shutil.copyfile(src_filename, dst_filename)
        fp_read = open(src_filename, 'r')
